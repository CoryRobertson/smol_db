...
impl ClientKey {
    ...
    pub fn get_pub_key(&self) -> &RsaPublicKey {
        self.pub_key.get_or_init(|| {
            info!("Generating client key public key");
            self.get_pri_key().to_public_key()
        })
    }

    #[tracing::instrument]
    fn get_pri_key(&self) -> &RsaPrivateKey {
    ...
    pub fn decrypt_server_packet(
        ...
        let msg =
            decrypt(self.get_pri_key(), server_db_response).map_err(EncryptionError::RSAError)?;
    ...
    pub fn decrypt(&self, msg: &[u8]) -> rsa::Result<Vec<u8>> {
        self.get_pri_key().decrypt(Pkcs1v15Encrypt, msg)
    }
